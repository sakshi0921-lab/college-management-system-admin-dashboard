...
const Supervisor = () => {
  ...
  const [invoices, setInvoices] = useState([
    { id: 1, name: 'Dr. Shilpa Sachdeva ', phone: '123-456-7890', email: 'Shilpa@gmail.com' },
    // Add more initial invoices here...
  ]);

  const [newInvoice, setNewInvoice] = useState({ id: null, name: '', phone: '', email: '' });
  ...
  */
  ...
  const handleSave = () => {
    ...
    setNewInvoice({ id: null, name: '', phone: '', email: '' }); // Reset the form
  };
  ...
  return (
    <Box m="20px">
      <Header title="SUPERVISOR" subtitle="Add Supervisor" />
      <Box mt="20px" mb="20px">
        <Typography variant="h6" gutterBottom>{newInvoice.id ? 'Edit Invoice' : 'Add New Supervisor'}</Typography>
        <Box component="form" display="flex" gap="10px" flexWrap="wrap" onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
          ...
          {columns
            .filter(column => !['id', 'name', 'phone', 'email', 'actions'].includes(column.field))
            .map((column, index) => (
              <TextField
                key={index}
